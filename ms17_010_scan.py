...
$ python2.7 smb_exploit.py 192.168.206.1 192.168.206.254
[+] [192.168.206.152] is likely VULNERABLE to MS17-010! (Windows 7 Ultimate 7600)
...
def tree_connect_andx_request(ip, userid):
    ...
    ipc = "\\\\{0}\IPC$\x00".format(ip)
    log.debug("Connecting to {0} with UID = {1}".format(ipc, userid))
...
def check(ip, port=445):
    ...
    try:
        ...
        if nt_status == '\x05\x02\x00\xc0':
            log.info("[+] [{0}] is likely VULNERABLE to MS17-010! ({1})".format(ip, native_os))
            ...
            if smb.multiplex_id == 0x0051:
              ...
              log.info("Host is likely INFECTED with DoublePulsar! - XOR Key: {0}".format(key))

        elif nt_status in ('\x08\x00\x00\xc0', '\x22\x00\x00\xc0'):
            log.info("[-] [{0}] does NOT appear vulnerable".format(ip))
        else:
            log.info("[-] [{0}] Unable to detect if this host is vulnerable".format(ip))
        ...
        # if  err==None:
        #     err='Patched!'
        log.error("[-] [{0}] Exception: {1}".format(ip, err))
        # continue
...
# python2 smb_ms17_010_scan.py [start_ip][end_ip]
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3:
        if len(sys.argv) == 2:
            ...
            print("python2 smb_ms17_010_scan.py [start_ip][end_ip]")
            print("python2 smb_ms17_010_scan.py [single_ip]")
            sys.exit(1)
    else:
        lines = iplist(sys.argv[1],sys.argv[2])
        for line in lines:
            # print line
            check(line)
